...
def ensure_self_signed(cert_path: str | Path, key_path: str | Path) -> Tuple[str, str]:
    ...
    if CRYPTO_AVAILABLE:  # type: ignore
        ...
        cert = (
            ...
            .add_extension(x509.SubjectAlternativeName([x509.DNSName("localhost")]), critical=False)  # type: ignore
    ...
    try:
        ...
        raise RuntimeError(
            "OpenSSL not found on PATH and 'cryptography' unavailable; install 'cryptography' or provide cert/key manually."
        ) from e
    return str(cert_path), str(key_path)
