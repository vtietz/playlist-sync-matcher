...
Implements OAuth 2.0 PKCE flow for Spotify API access.
Handles token acquisition, refresh, and caching.

Moved from psm.auth.spotify_oauth to encapsulate all Spotify logic in the provider package.
"""
...
try:
    # Local import cert utility from auth package (provider-agnostic)
    from ...auth.certutil import ensure_self_signed  # type: ignore
    ...
    def clear_cache(self) -> None:
        ...
        if os.path.exists(self.cache_file):
            try:
                ...
                logger.debug(f"[auth] Cleared token cache: {self.cache_file}")
            except Exception as e:
                logger.warning(f"[auth] Failed to clear cache {self.cache_file}: {e}")
