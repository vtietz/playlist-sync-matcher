...
def ingest_playlists(db, client: "SpotifyAPIClient", use_year: bool = False, force_refresh: bool = False):
    ...
    for pl in client.current_user_playlists():
        ...
        existing_playlist = db.conn.execute("SELECT snapshot_id FROM playlists WHERE id = ?", (pid,)).fetchone()
        ...
        for idx, item in enumerate(tracks):
            ...
            if not existing_track:
                ...
                if (
                    existing_track["name"] != track_data["name"]
                    or existing_track["artist"] != track_data["artist"]
                    or existing_track["album"] != track_data["album"]
                    or existing_track["isrc"] != track_data["isrc"]
                    or existing_track["duration_ms"] != track_data["duration_ms"]
                    or existing_track["year"] != track_data["year"]
                ):
                    is_new_or_changed = True
        ...
        logger.info(
            f"{click.style(f'[{action}]', fg=color)} {name} ({len(simplified)} tracks) | owner={owner_name or owner_id or 'unknown'}"
        )
