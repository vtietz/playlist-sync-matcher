...
def playlist_build(ctx: click.Context, playlist_id: str, force_auth: bool):
    ...
    if not provider_cfg.get("client_id"):
        raise click.UsageError("providers.spotify.client_id not configured")
    with get_db(cfg) as db:
        result = build_single_playlist(
            db=db, playlist_id=playlist_id, spotify_config=provider_cfg, config=cfg, force_auth=force_auth
        )
...
@click.option(
    ...
    required=False,
    help="Exported M3U file (file mode). Omit for DB mode.",
)
