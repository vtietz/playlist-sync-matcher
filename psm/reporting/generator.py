...
def write_analysis_quality_reports(
...
def write_match_reports(
    db: Database, out_dir: Path, affected_playlist_ids: List[str] | None = None
...
def _generate_playlist_details(
    db: Database, out_dir: Path, provider: str, affected_playlist_ids: List[str] | None = None
) -> None:
    ...
    if affected_playlist_ids is not None:
        ...
        for playlist_id in affected_playlist_ids:
            playlist = db.conn.execute("SELECT id, name FROM playlists WHERE id = ?", (playlist_id,)).fetchone()
            if playlist:
