...
from ..formatting import format_duration, format_playlist_count_simple
from ..html_templates import get_html_template
from .base import format_liked


def write_unmatched_tracks_report(db: Database, out_dir: Path, provider: str = "spotify") -> tuple[Path, Path]:
...
def _write_html(html_path: Path, unmatched_rows: list, provider: str) -> None:
    ...
    for row in unmatched_rows:
        ...
        track_link = (
            f'<a href="{track_url}" target="_blank" title="Open in {provider.title()}">{row["name"] or "Unknown"}</a>'
        )
