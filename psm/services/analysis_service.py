...
def analyze_library_quality(
    db: DatabaseInterface, min_bitrate_kbps: int = 320, max_issues: int = 50, silent: bool = False
) -> QualityReport:
    """
    ...
    if not silent:
        logger.info(
            f"[analyze] Analysis complete. Found issues in {len(report.issues)} files (showing first {max_issues})"
        )
    ...
    if db and (stats["missing_artist"] > 0 or stats["missing_album"] > 0 or stats["missing_year"] > 0):
        ...
        if stats["missing_year"] > 0:
            ...
            for group in offenders:
                ...
                logger.info(
                    f"    {click.style(f'📁 {artist} - {album}', fg='cyan')} ({count} file{'s' if count != 1 else ''})"
                )
            ...
            if stats["missing_year"] > total_files_shown:
                logger.info(
                    f"    ... and {stats['missing_year'] - total_files_shown} more files in {stats['missing_year'] - total_files_shown} other albums"
                )
            logger.info("")
