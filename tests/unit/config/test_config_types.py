...
def test_matching_config_custom_values():
    ...
    config = MatchingConfig(fuzzy_threshold=0.85, duration_tolerance=3.0, max_candidates_per_track=300)
...
def test_app_config_from_dict():
    ...
    raw_config = {
        "log_level": "DEBUG",
        "provider": "spotify",
        "providers": {"spotify": {"client_id": "test123", "redirect_port": 8888}},
        "matching": {"fuzzy_threshold": 0.85, "duration_tolerance": 3.0},
        "library": {"paths": ["/music", "/media/music"], "min_bitrate_kbps": 256},
...
def test_app_config_roundtrip():
    ...
    original = AppConfig(log_level="DEBUG", matching=MatchingConfig(fuzzy_threshold=0.90))
...
def test_providers_config_to_dict():
    ...
    providers = ProvidersConfig(spotify=SpotifyConfig(client_id="test123", redirect_port=8080))
