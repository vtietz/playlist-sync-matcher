...
def temp_db(tmp_path: Path):
    ...
    db.replace_playlist_tracks("playlist1", [(0, "track1", None), (1, "track2", None)], provider="spotify")

    db.replace_playlist_tracks("playlist2", [(0, "track2", None)], provider="spotify")

    db.replace_playlist_tracks("playlist3", [(0, "track3", None)], provider="spotify")
...
class TestWatchModeIncrementalPipeline:
    ...
    def test_handle_library_changes_scoped_export(
        ...
        watch_config = WatchBuildConfig(
            config=config, get_db_func=lambda cfg: temp_db, skip_export=False, skip_report=False
        )
    ...
    def test_export_playlists_helper_skips_on_empty_list(self, temp_db: Database, tmp_path: Path):
        ...
        config = {"export": {"directory": str(tmp_path / "export"), "organize_by_owner": False}}
    ...
    def test_generate_reports_helper_skips_on_empty_list(self, temp_db: Database, tmp_path: Path):
        ...
        config = {"reports": {"directory": str(tmp_path / "reports")}}
