...
def db_with_data(tmp_path):
    ...
    db.upsert_playlist(
        pid="p1", name="Workout Mix", snapshot_id=None, owner_id="user1", owner_name="testuser", provider="spotify"
    )
    db.upsert_playlist(
        pid="p2", name="Chill Vibes", snapshot_id=None, owner_id="user1", owner_name="testuser", provider="spotify"
    )

    # Add tracks (upsert_track expects a dict with flat fields)
    db.upsert_track(
        track={"id": "t1", "name": "Song 1", "artist": "Artist 1", "album": "Album 1", "duration_ms": 180000},
        provider="spotify",
    )
    db.upsert_track(
        track={"id": "t2", "name": "Song 2", "artist": "Artist 2", "album": "Album 2", "duration_ms": 200000},
        provider="spotify",
    )
    ...
    db.add_match(track_id="t1", file_id=file_id, score=95.0, method="exact", provider="spotify")
