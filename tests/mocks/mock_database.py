...
class MockDatabase(DatabaseInterface):
    ...
    def get_missing_tracks(self) -> Iterable[Any]:
        ...
        for (tid, prov), data in self.tracks.items():
            if tid not in matched_ids:
                yield MockRow(
                    {"id": tid, "name": data.get("name"), "artist": data.get("artist"), "album": data.get("album")}
                )
    ...
    def get_playlist_coverage(self, provider: str | None = None) -> List[Dict[str, Any]]:
        ...
        for (pid, prov), playlist in self.playlists.items():
            ...
            matched = sum(
                1
                for _, tid, _ in tracks_in_playlist
                if any(m.get("track_id") == tid and m.get("provider") == prov for m in self.matches)
            )
            result.append(
                {
                    "id": pid,
    ...
    def list_unified_tracks_min(
        ...
        for (tid, prov), track in self.tracks.items():
            if prov != provider:
                continue
            # Find match
            match = next((m for m in self.matches if m.get("track_id") == tid and m.get("provider") == prov), None)
