...
def test_match_idempotent(tmp_path: Path):
    ...
    # Insert track + file
    db.upsert_track({'id': 't1', 'name': 'N', 'album': 'A', 'artist': 'R', 'isrc': None, 'duration_ms': 1000, 'normalized': 'n r'})
    ...
    db.add_match('t1', file_id, 0.9, 'fuzzy')
    db.add_match('t1', file_id, 1.0, 'exact')  # update existing
